...
# <Printer className='inline w-[1em] h-[1em] mr-1 -mt-1' /> 日替わり表の印刷
...
<Callout type="warn" title="コンテンツの有効化" icon={<AlertTriangle />}>
	画面上部に「セキュリティの警告」という黄色いバーが出ている場合は、**「コンテンツの有効化」**
	をクリックしてください。これを行わないとボタンが反応しません。
</Callout>
...
![日付ボタンの場所](/images/Date_Button.png)
...
![印刷ボタンの場所](/images/Print_Button.png)
...
<IconList icon="question" color="text-green-600">
	<Item>
    ...
    <Item>**日付は正しいですか？**（伝票用が「翌日の日付」になっているか）</Item>
    <Item>**印刷は鮮明ですか？**（インク切れでかすれていないか）</Item>
...
## <MessageCircleWarning className='inline w-[1em] h-[1em] mr-1 -mt-1' /> トラブルシューティング
...
<Accordions>
    ...
    <Accordion title="エラー画面が表示された" icon={<AlertTriangle />}>
    	**対処法:**
    	...
    	3. それでも直らない場合は、システム担当（{SITE_CONFIG.admin}）へ連絡してください。
    </Accordion>
