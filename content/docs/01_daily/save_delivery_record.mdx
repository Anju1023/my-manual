...
# <Save className='inline w-[1em] h-[1em] mr-1 -mt-1' /> 納品記録の保存

翌日納品分の売上データを確定させ、記録用の PDF ファイルを保存します。\
「日替わり表の印刷」が完了した後に、手が空いたタイミングで実施してください（当日中）。
    	...
    	<Folder name='デスクトップ' defaultOpen>
    		<Folder name='よく使う' defaultOpen>
    			...
    			<Folder name='伝票作成' defaultOpen>
    				<Folder name='納品記録' defaultOpen>
    					<Folder name='納品記録PDF' defaultOpen>
...
<Steps>
    ...
    <Step>
    	...
    	![PDF保存ボタンの場所](/images/PDF_Button.png)
    ...
    <Step>
    	...
    	3.  **以下のポイントをチェックしてください：**
    		...
    		<IconList color="text-green-600">
    			<Item>店舗ごとの PDF ファイルが揃っていますか？</Item>
    			<Item>
    				**ランダムに 1 つ開いて、金額が `0円` になっていませんか？**

    				- （※もし0円の場合は、マスタ登録漏れの可能性があります）
    					![PDFをチェック](/images/PDF_Check.png)
...
<Accordions>
    ...
    <Accordion title="エラーで止まってしまった" icon={<AlertTriangle />}>
    	...
    	を押してファイルを閉じてください。\
    	その後、システム担当（{SITE_CONFIG.admin}）へ状況を報告してください。
    </Accordion>
